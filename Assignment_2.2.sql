...
SELECT 
    p.national_ID, p.stud_ID, p.stud_name, p.phone_number, p.age, p.gender,
...
SET fee_cleared = CASE WHEN sem_fee <= fee_paid THEN TRUE ELSE FALSE END;
...
SELECT 
    ...
    SUM(CASE WHEN sem_fee > fee_paid THEN sem_fee - fee_paid ELSE 0 END) AS total_deficit
...
ORDER BY s.secondary_school_county;
